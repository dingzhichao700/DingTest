...
//---- STATIC FUNCTIONS -------------------------------------------------------------------------
		...
		/**
		 * @param targets Target object array whose properties this tween affects. This can be ANY object, not just a DisplayObject. 
		 * @param duration Duration in seconds (or frames if "useFrames" is true)
		 * @param vars An object containing the end values of the properties you're tweening, like {x:100, y:50}. It can also contain special properties like "onComplete", "ease", "delay", etc.
		 */ 
		public static function allTo(targets:Array, duration:Number, vars:Object):void
		{
			for (var i:int = 0; i < targets.length; i++)
			{
				new TweenNano(targets[i], duration, vars); 
			}
		}
		 ...
		 * @param current Stop target at the current position.
		 */
		 public static function killTweensOf(target:Object, complete:Boolean=false, current:Boolean = false):void {
			if (target in _masterList) {
				if (complete) {
					var a:Array = _masterList[target];
					var i:int = a.length;
					while (i--) {
						if (!TweenNano(a[i]).gc && !(target is Function)) {
							if (!current) {
		...
		/**
		 * Kills all the tweens of a particular object array, optionally forcing them to completion too.
		 * 
		 * @param targets Object array whose tweens should be immediately killed
		 * @param complete Indicates whether or not the tweens should be forced to completion before being killed.
		 * @param current Stop target at the current position.
		 */
		public static function killAllTweensOf(targets:Array, complete:Boolean = false, current:Boolean = false):void
		{
			for (var i:int = 0; i < targets.length; i++)
			{
				killTweensOf(targets[i], complete, current);
			}
		}
